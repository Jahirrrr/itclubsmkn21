...
export default class GroupsNav extends React.Component {
        ...
        fire.auth().onAuthStateChanged((user) => {
            if (user) {
                fire.firestore().collection('users').doc(user.uid).get().then(userData => {
                    if (userData.data()) {
                        const sortedGroupsArr = userData.data().joinedGroups.sort((a, b) => a.toLowerCase() > b.toLowerCase())
                        this.setState({
                            groups: sortedGroupsArr
                        })
                    } else {
                        this.setState({
                            groups: []
    ...
    render() {
        return (
            <div id='groupList'>
                <button onClick={this.displayGroups}>Group</button>
